                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <StatCard>
                        ...
                        <template #content>
                            <div class="h-32">
                                <DoughnutChart 
                                    v-if="workOrderStatusChart && workOrderStatusChart.data && workOrderStatusChart.data.length" 
                    ...
                    <StatCard class="lg:col-span-2">
                        ...
                        <template #content>
                            <ul v-if="topProducts.length > 0" class="space-y-3">
                                <li v-for="product in topProducts" :key="product.name" class="flex justify-between items-center text-sm">
                                    <span class="text-gray-700">{{ product.name }}</span>
                                    <span class="font-semibold text-gray-900">{{ formatCurrency(product.total_revenue) }}</span>
                                </li>
                            </ul>
                    ...
                    <StatCard class="md:col-span-2 lg:col-span-3">
                        ...
                        <template #content>
                            <div class="h-64">
                                <BarChart 
                                    v-if="dailyActivityChart && dailyActivityChart.data && dailyActivityChart.data.length" 
                                    :chart-data="barChartData" 
                                />
                                <p v-else class="text-gray-500 text-center py-16">No hay actividad registrada en los últimos 30 días.</p>
                            </div>
                        </template>
                    </StatCard>
                    ...
                    <StatCard>
                        ...
                        <template #content>
                            <ul v-if="lowStockProducts.length > 0" class="space-y-3">
                                <li v-for="product in lowStockProducts" :key="product.name" class="flex justify-between items-center text-sm">
                                    <span class="text-gray-700">{{ product.name }}</span>
                                    <span class="font-semibold text-red-600">
                                        {{ product.current_stock }} / <span class="text-gray-500">{{ product.min_threshold }}</span>
                                    </span>
                                </li>
                            </ul>
